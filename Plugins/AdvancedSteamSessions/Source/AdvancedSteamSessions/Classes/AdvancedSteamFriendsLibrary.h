...
// UE4 - OnlineSubsystemSteamPrivatePCH.h 에서 직접 가져온 것으로, array_count 매크로에 대한 픽스입니다
...
public:
	...
	*/
	...
	// 지정된 필터링 컨텍스트를 사용하여 문자열을 필터링하려고 시도합니다.
	// 텍스트가 필터링된 경우 true를 반환하고, 필터링되지 않은 경우 false를 반환합니다(필터링이 필요하지 않거나 작업이 실패함).
	// false이면 원본 텍스트가 계속 출력됩니다
	// Textsource는 텍스트의 소스인 Steam ID입니다(플레이어 이름 / 채팅).
	// InitTextFiltering을 먼저 호출해야 합니다!!
	UFUNCTION(BlueprintCallable, Category = "Online|SteamAPI|TextFiltering")
