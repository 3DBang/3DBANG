...
DEFINE_LOG_CATEGORY_STATIC(FindSessionsCallbackProxyAdvanced, Log, All);
...
void UFindSessionsCallbackProxyAdvanced::Activate()
{
	...
	if (Helper.IsValid())
	{
		...
		if (Sessions.IsValid())
		{
			...
			FOnlineSearchSettings DebugSearchSettings = SearchObject->QuerySettings;
			FSearchParams SearchParams = DebugSearchSettings.SearchParams;
